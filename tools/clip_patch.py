"""
This is a tool to clip the original .TIF images to small patches to build a dataset.2222
"""
...
def parse_args():
    ...
    parser.add_argument('-d', '--data_dir', required=True, help='root of data directory')           #源数据集
    parser.add_argument('-s', '--satellite', required=True, help='name of the satellite/dataset')   #卫星名
    parser.add_argument('-n', required=True, type=int, help='total number of images pairs')         #总图像对数 WV->3 GF->7
    parser.add_argument('-p', '--patch_num', required=True, type=int,
                        help='random clip how many patches for one training scene')                 #一个图中的patch数 wv->6000 gf->6000 qb->3000
            ...
            for y in range(0, YSize, patch_size // 2):
                ...
                for x in range(0, XSize, patch_size // 2):
                    if x + patch_size > XSize:
                        continue
                    save_image(f'{out_dir}/{cnt}_mul.tif',
                               img_mul[:, y * 4:(y + 32) * 4, x * 4:(x + 32) * 4])
                    save_image(f'{out_dir}/{cnt}_lr_u.tif',
                               img_lr_u[:, y * 4:(y + 32) * 4, x * 4:(x + 32) * 4])
                    save_image(f'{out_dir}/{cnt}_lr.tif',
                               img_lr[:, y:(y + 32), x:(x + 32)])
                    save_image(f'{out_dir}/{cnt}_pan.tif',
                               img_pan[y * 4:(y + 32) * 4, x * 4:(x + 32) * 4])
                    cnt += 1
                    col += 1
                row += 1
            record.write("%d: %d * %d\n" % (num, row, col))
