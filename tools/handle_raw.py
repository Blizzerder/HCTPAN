"""
This is a tool to preprocess the original giant remote sensing .TIF images.1111
"""
...
def parse_args():
    ...
    parser.add_argument('-d', '--data_dir', required=True, help='root of data directory')           #数据根目录
    parser.add_argument('-s', '--satellite', required=True, help='name of the satellite/dataset')   #卫星名
    parser.add_argument('-n', required=True, type=int, help='total number of images pairs')         #总共要切几块 WV->3
    parser.add_argument('-m', '--mode', choices=['Gaussian', 'Bicubic'], default='Gaussian',
                        help='type of interpolation')                                               #插值方法Gaussian
    parser.add_argument('--reduce_raw', action='store_true',
                        help='sometimes the raw resolution image for testing is too big, '
                             'use this to reduce to 1/4')                                           #将原图缩小至多少
...
if __name__ == "__main__":
    args = parse_args()
    in_dir = f'{args.data_dir}/{args.satellite}'                    #输入文件夹的位置
    out_dir = f'{args.data_dir}/Dataset/{args.satellite}'           #输出文件夹的位置
    mmcv.mkdir_or_exist(out_dir)                                    #创建文件夹
    ...
    for i in range(0, args.n):
        ...
        imgPan = rawPan
        imgPan = down_sample(imgPan, mode=args.mode)  # h * w * 1
